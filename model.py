...
if __name__ == '__main__':
    ...
    model = TuduiModel()
    output = model(input)
    print("Output shape:", output.shape)  # 应该是 torch.Size([1, 5])
    print("Output range:", torch.min(output).item(), torch.max(output).item())  # 应该在0-1之间
    print("Sample output:", output[0])  # 查看一个样本的输出